...
module Schema = {
  ...
  let field: 'a. (string, Validator.t<'a>) => t<'a> = (name, validator) =>
    liftF(Field.make(name, validator))
...
let validateUser = (first: string, last: string, age: string): ValidationE.t<User.t> => {
  ...
  module SchemaValidate = Schema.WithApplicativeAndNT(ValidationE.Applicative, NT)
