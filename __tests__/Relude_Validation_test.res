...
module ValidationE = Validation.WithErrors(NonEmptyList.SemigroupAny, Error.Type)
...
module Person = {
  ...
  let makeWithNoValidation: (string, int, string) => t = (name, age, language) => {
    name,
    age,
    language,
  }

  let validateName: string => Validation.t<string, errors> = name =>
  ...
  let validateAge: int => Validation.t<int, errors> = age =>
    if age < 0 || age > 120 {
  ...
  let validateLanguage: string => Validation.t<string, errors> = lang =>
...
describe("Validation", () => {
  ...
  test("isError success", () => expect(Validation.VError("error")->Validation.isError)->toBe(true))
  ...
  testAll(
    ...
    list{
      (Validation.ok(42), Validation.ok("a"), Validation.ok(Relude_Ior_Type.Both(42, "a"))),
      (Validation.ok(42), Validation.error("fail2"), Validation.ok(Relude_Ior_Type.This(42))),
      (Validation.error("fail1"), Validation.ok("a"), Validation.ok(Relude_Ior_Type.That("a"))),
      (Validation.error("fail1"), Validation.error("fail2"), Validation.error("fail1fail2")),
    },
  ...
  test("makeWithValidation two errors", () => {
    ...
    let expected: errors = NonEmptyList.make(
