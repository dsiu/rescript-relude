...
module StorageF = {
  ...
  module WithKeyAndValue = (K: BsBastet.Interface.TYPE, V: BsBastet.Interface.TYPE) => {
    type t<'a> = t<K.t, V.t, 'a>
    ...
    let get: K.t => FreeMonad.t<option<V.t>> = key => FreeMonad.liftF(Get(key, a => a))

    let put: (K.t, V.t) => FreeMonad.t<unit> = (key, value) => FreeMonad.liftF(Put(key, value, ()))

    let delete: K.t => FreeMonad.t<unit> = key => FreeMonad.liftF(Delete(key, ()))
...
module StorageAPI = {
  ...
  module State = Relude.StateT.State.WithState({
    type t = Belt.Map.String.t<int>
  })

  // Low-level operations that we'll use for our interpreter
  let getState: string => State.t<option<int>> = key => StateT(
    s => (Belt.Map.String.get(s, key), s),
  )

  let putState: (string, int) => State.t<unit> = (key, value) => StateT(
    s => ((), Belt.Map.String.set(s, key, value)),
  )

  let deleteState: string => State.t<unit> = key => StateT(
    s => ((), Belt.Map.String.remove(s, key)),
  )

  // Storage algebra specialized to string key and int value
  module StorageFWithKeyAndValue = StorageF.WithKeyAndValue(
    {
      type t = string
    },
    {
  ...
  let interpreter: StorageFWithKeyAndValue.t<'a> => State.t<'a> = storage => {
    open State.Infix
    switch storage {
  ...
  describe("StorageAPI", () =>
    test(
      ...
      () => {
        ...
        let (results, _) = StorageAPI.State.runStateT(
