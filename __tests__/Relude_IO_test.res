...
describe("IO basics", () => {
  ...
  testAsync("throw unsafeRunAsync", onDone =>
    IO.throwWithVoid("this is a test") |> IO.unsafeRunAsync(x =>
      switch x {
...
describe("IO tries/exceptions", () => {
  testAsync("tries unsafeRunAsync", onDone =>
    IO.tries(throwJSError) |> IO.unsafeRunAsync(x =>
      switch x {
  ...
  testAsync("triesJS unsafeRunAsync", onDone =>
    IO.triesJS(throwJSError) |> IO.unsafeRunAsync(x =>
      switch x {
      | Ok(_) => onDone(fail("Should not be Ok"))
      | Error(jsExn) =>
        let msg = Js.Exn.message(jsExn)
  ...
  Skip.testAsync("triesJS with exn", onDone => {
    ...
    IO.triesJS(() => raise(MyExn("Custom error"))) |> IO.unsafeRunAsync(result =>
      switch result {
      | Ok(_) => onDone(fail("Should not be Ok"))
      | Error(e) =>
        Js.log(e)
...
describe("IO delay", () => {
  testAsync("delay unsafeRunAsync", onDone => {
    IO.delay(10) |> IO.unsafeRunAsync(x =>
      switch x {
      | Ok(_) => onDone(pass)
      | Error(_) => onDone(fail("Failed"))
      }
...
describe("IO debounce", () => {
  ...
  Skip.testAsync("debounce", onDone => {
    ...
    let debouncedIO = IO.debounce(~intervalMs, () => {
    ...
    let checkNonRunIO = IO.unsafeRunAsync(
      Result.fold(
        ...
        Relude_Option.foldLazy(ignore, () => "IO should not have been run" |> fail |> onDone),
      ),
    )
    ...
    debouncedIO() |> IO.flatMap(Relude_Option.fold(IO.pure(None), debouncedIO)) |> checkNonRunIO
  ...
  Skip.testAsync("debounce immediate", onDone => {
    ...
    let debouncedIO = IO.debounce(~immediate=true, ~intervalMs, () => {
      timeIntervals := list{getTimestamp(), ...timeIntervals.contents}
      IO.pure()
    })
...
describe("IO throttle", () => {
  ...
  Skip.testAsync("throttle", onDone => {
    ...
    let timeIntervals = ref(list{getTimestamp()})
    let intervalMs = 100
    let throttledIO = IO.throttle(~intervalMs, () => {
      timeIntervals := list{getTimestamp(), ...timeIntervals.contents}
      IO.pure()
    })
    ...
    throttledIO() |> IO.flatMap(\">>"(ignore, throttledIO)) |> IO.unsafeRunAsync(ignore)
...
describe("IO realish examples", () => {
  testAsync("example >>=", onDone =>
    \">>="(
      \">>="(
        \">>="(getData |> IO.mapError(eGet), \">>"(parseData, IO.mapError(eParse))),
  ...
  testAsync("example >=>", onDone => {
    let getIO = \">=>"(
      \">=>"(
        \">=>"(_ => getData |> IO.mapError(eGet), \">>"(parseData, IO.mapError(eParse))),
        \">>"(printNumber, IO.mapError(ePrint)),
      ),
      _ => IO.pure(pass),
    )

    getIO() |> IO.unsafeRunAsync(x =>
      switch x {
      | Ok(assertion) => onDone(assertion)
      | Error(_) => onDone(fail("Failed"))
      }
  ...
  testAsync("readFile", onDone =>
    \">>="(
      \">>="(
        ...
        content =>
          Relude_String.toNonWhitespace(content) |> IO.fromOption(_ =>
