...
describe("Option", () => {
  ...
  test("orElse (primary is some)", () =>
    expect(Some(3) |> Option.orElse(~fallback=Some(1))) -> toEqual(Some(3))
  )

  test("tap (none)", () => {
    let x = ref(1)
    let _ = Option.tap(v => x := x.contents + v, None)
    expect(x.contents) -> toEqual(1)
  })

  test("tap (some)", () => {
    let x = ref(1)
    let _ = Option.tap(v => x := x.contents + v, Some(3))
    expect(x.contents) -> toEqual(4)
  })

  test("tapSome (none)", () => {
    let x = ref(1)
    let _ = Option.tapSome(v => x := x.contents + v, None)
    expect(x.contents) -> toEqual(1)
  })

  test("tapSome (some)", () => {
    let x = ref(1)
    let _ = Option.tapSome(v => x := x.contents + v, Some(-1))
  ...
  test("toList has one item when option is Some", () =>
    expect(Option.toList(Some("a"))) -> toEqual(list{"a"})
  )

  test("toList is empty when option is None", () => expect(Option.toList(None)) -> toEqual(list{}))

  test("toArray has one item when option is Some", () =>
    expect(Option.toArray(Some([0]))) -> toEqual([[0]])
  )

  test("toArray is empty when option is None", () => expect(Option.toArray(None)) -> toEqual([]))

  test("isSome is true for Some", () => expect(Option.isSome(Some(3))) -> toEqual(true))

  test("isSome is false for None", () => expect(Option.isSome(None)) -> toEqual(false))

  test("isNone is true for None", () => expect(Option.isNone(None)) -> toEqual(true))

  test("isNone is false for Some", () => expect(Option.isNone(Some(0))) -> toEqual(false))

  test("map", () => expect(Option.map(a => a + 2, Some(1))) -> toEqual(Some(3)))

  test("apply", () => expect(Option.apply(Some(a => a + 2), Some(1))) -> toEqual(Some(3)))
  ...
  testAll(
    ...
    ((fa, fb, expected)) => expect(Option.align(fa, fb)) -> toEqual(expected),
  )
  ...
  test("pure", () => expect(Option.pure(5)) -> toEqual(Some(5)))

  test("bind", () => expect(Option.bind(Some(1), a => Some(a + 2))) -> toEqual(Some(3)))
  ...
  test("map2", () => expect(Option.map2((a, b) => a + b, Some(1), Some(2))) -> toEqual(Some(3)))

  test("map3", () =>
    expect(Option.map3((a, b, c) => a + b + c, Some(1), Some(2), Some(3))) -> toEqual(Some(6))
  )

  test("map4", () =>
    expect(
      Option.map4((a, b, c, d) => a + b + c + d, Some(1), Some(2), Some(3), Some(4)),
    ) -> toEqual(Some(10))
  )
  ...
  test("tuple2", () => expect(Option.tuple2(Some(1), Some(2))) -> toEqual(Some((1, 2))))

  test("tuple3", () => expect(Option.tuple3(Some(1), Some(2), Some(3))) -> toEqual(Some((1, 2, 3))))

  test("tuple4", () =>
    expect(Option.tuple4(Some(1), Some(2), Some(3), Some(4))) -> toEqual(Some((1, 2, 3, 4)))
  )

  test("tuple5", () =>
    expect(Option.tuple5(Some(1), Some(2), Some(3), Some(4), Some(5))) -> toEqual(
      Some((1, 2, 3, 4, 5)),
    )
  )

  test("mapTuple2", () =>
    expect((Some(1), Some(2)) |> Option.mapTuple2((a, b) => a + b)) -> toEqual(Some(3))
  )

  test("mapTuple3", () =>
    expect((Some(1), Some(2), Some(3)) |> Option.mapTuple3((a, b, c) => a + b + c)) -> toEqual(
      Some(6),
    )
  )
  ...
  test("mapTuple4", () =>
    expect(
      (Some(1), Some(2), Some(3), Some(4)) |> Option.mapTuple4((a, b, c, d) => a + b + c + d),
    ) -> toEqual(Some(10))
  )

  test("mapTuple5", () =>
    expect(
      (Some(1), Some(2), Some(3), Some(4), Some(5)) |> Option.mapTuple5((a, b, c, d, e) =>
        a + b + c + d + e
      ),
    ) -> toEqual(Some(15))
  )
  ...
  test("foldLeft", () =>
    expect(Option.foldLeft((acc, v) => list{v, ...acc}, list{10, 20}, Some(1))) -> toEqual(list{
      1,
      10,
      20,
    })
  ...
  test("foldRight", () =>
    expect(Option.foldRight((v, acc) => list{v, ...acc}, list{10, 20}, Some(1))) -> toEqual(list{
      1,
      10,
      20,
    })
  ...
  test("alt both", () => expect(Option.alt(Some(1), Some(2))) -> toEqual(Some(1)))

  test("alt left", () => expect(Option.alt(Some(1), None)) -> toEqual(Some(1)))

  test("alt right", () => expect(Option.alt(None, Some(1))) -> toEqual(Some(1)))

  test("alt neither", () => expect(Option.alt(None, None)) -> toEqual(None))

  test("empty", () => expect(Option.empty) -> toEqual(None))
  ...
  test("filterNot", () =>
    expect(Option.filterNot(x => mod(x, 2) == 0, Some(1))) -> toEqual(Some(1))
  )

  test("flatten is None when option is None", () => expect(Option.flatten(None)) -> toEqual(None))
...
describe("Option Specializations", () => {
  test("String.eq (both none)", () => expect(Option.String.eq(None, None)) -> toEqual(true))
  ...
  test("String.compare (both some, same value)", () =>
    expect(Option.String.compare(Some("hi"), Some("hi"))) -> toEqual(#equal_to)
  )

  test("String.compare (both some, different value)", () =>
    expect(Option.String.compare(Some("b"), Some("a"))) -> toEqual(#greater_than)
  )

  test("Int.eq", () => expect(Option.Int.eq(Some(1), None)) -> toEqual(false))

  test("Float.eq", () => expect(Option.Float.eq(Some(3.14), Some(3.14))) -> toEqual(true))
})
