...
describe("Json", () => {
  ...
  test("validateOptionalAtIndex missing value is a None when missingAsNone=true", () =>
    expect(
      JD.optAt(
        ~missingAsNone=true,
        ~nullAsNone=false,
  ...
  test("validateOptionalForKey missing value is a None when missingAsNone=true", () =>
    expect(
      JD.optFor(
        ~missingAsNone=true,
        ~nullAsNone=false,
  ...
  test("decode array using applicative validation (success)", () => {
    let actual: Validation.t<array<string>, NonEmptyArray.t<string>> =
      JE.array([JE.string("hi"), JE.string("bye")])->(JD.array((_, json) => JD.string(json), _))
  ...
  test("decode object using applicative validation (success)", () => {
    let json: Js.Json.t = JE.listOfTuples(list{
      ...
      ("b", JE.int(42)),
      ("c", JE.bool(true)),
      ("d", JE.listBy(JE.string, list{"one", "two"})),
      ("e", JE.float(3.14)),
      ("f", JE.null),
      ("g", JE.listBy(JE.string, list{"three", "four"})),
    })

    let actual: Validation.t<MyType.t, NonEmptyArray.t<string>> = // flatMap a value
    // map a value
    // todo: myType.make need to be uncurried with 7 args....this is tidious
    \"<*>"(
      \"<*>"(
        \"<*>"(
          \"<*>"(
            \"<*>"(
              \"<*>"(
                \"<$>"(
                  a => b => c => d => e => f => g => MyType.make(a, b, c, d, e, f, g),
                  \">>="(JD.stringFor("a", json), a => VOk(a ++ a)),
                ),
                \"<$$>"(JD.intFor("b", json), a => a * 2),
              ),
              JD.boolFor("c", json),
            ),
  ...
  test("decode object using applicative validation (invalid type error)", () => {
    ...
    // todo: myType.make need to be uncurried with 3 args....this is tidious
    let actual = \"<*>"(
      \"<*>"(\"<$>"(a => b => c => make(a, b, c), JD.intFor("a", json)), JD.floatFor("b", json)),
      JD.listFor("c", (_index, json) => JD.string(json), json),
    )
    let expected = Validation.error(
      NonEmptyArray.make(
        "a: JSON value is not an int: null",
        ["b: JSON value is not a float: null", "0: JSON value is not a string: 1"],
      ),
    )
        ...
        \"<*>"(
          \"<*>"(
            \"<*>"(
              \"<*>"(
                \"<$>"(
                  a => b => c => d => e => f => g => make(a, b, c, d, e, f, g),
                  JD.stringFor("a", json),
                ),
                JD.intFor("b", json),
              ),
  ...
  test("decode an array into an object (success)", () => {
    let json = JE.array([
      ...
      JE.int(42),
      JE.bool(true),
      JE.list(list{JE.string("one"), JE.string("two")}),
      JE.float(3.14),
      JE.null,
      JE.list(list{JE.string("three"), JE.string("four")}),
    ])

    let actual = // bind (flatMap) a value here just for fun
    // map a value here - <$$> is flipMap - we need to flip it b/c the Validation comes first here
    ...
    \"<*>"(
          ...
          \"<*>"(
            \"<*>"(
              \"<*>"(
                \"<$>"(
                  a => b => c => d => e => f => g => MyType.make(a, b, c, d, e, f, g),
                  \">>="(JD.stringAt(0, json), a => VOk(a ++ a)),
                ),
                \"<$$>"(JD.intAt(1, json), a => a * 2),
              ),
              JD.boolAt(2, json),
            ),
      ...
      JD.jsonAt(6, JD.list((_index, json) => JD.string(json), _), json),
    )
  ...
  test("decode an array into a tuple (not found error)", () => {
    ...
    let make: (
      ...
      bool,
      array<string>,
    ) => (string, int, float, bool, array<string>) = (a, b, c, d, e) => (a, b, c, d, e)

    let actual = \"<*>"(
      \"<*>"(
        \"<*>"(
          \"<*>"(
            \"<$>"(a => b => c => d => e => make(a, b, c, d, e), JD.stringAt(0, json)),
            JD.intAt(1, json),
          ),
          JD.floatAt(2, json),
        ),
