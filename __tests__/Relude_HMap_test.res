...
describe("HMap", () => {
  ...
  test("remove", () => {
    ...
    let map123 = HMap.empty |> HMap.add(intKey1, 1) |> HMap.add(intKey2, 2) |> HMap.add(intKey3, 3)
  ...
  test("fold", () => {
    ...
    let map =
      HMapS.empty |> HMapS.add(intKey, 5) |> HMapS.add(stringKey, "hi") |> HMapS.add(floatKey, 42.3)

    let value = map |> HMapS.fold((HMapS.KeyValue(k, v), acc) => {
  ...
  test("forEach", () => {
    ...
    map |> HMapS.forEach((HMapS.KeyValue(k, v)) => {
      // Use keyMeta to extract the key info - in this case our label, and the 'a => string function
      // that got embedded in the key
      let (label, show) = HMapS.Key.keyMeta(k)
  ...
  test("all", () => {
    ...
    let value = map |> HMapS.all((HMapS.KeyValue(k, _)) => {
      // Use keyMeta to extract the key info - in this case our label, and the 'a => string function
      // that got embedded in the key
  ...
  test("any", () => {
    ...
    let value = map |> HMapS.any((HMapS.KeyValue(k, _)) => {
      // Use keyMeta to extract the key info - in this case our label, and the 'a => string function
      // that got embedded in the key
      let (label, _) = HMapS.Key.keyMeta(k)
  ...
  test("filter", () => {
    ...
    let map2 = map |> HMapS.filter((HMapS.KeyValue(k, _)) => {
      // Use keyMeta to extract the key info - in this case our label, and the 'a => string function
      // that got embedded in the key
      let (label, _) = HMapS.Key.keyMeta(k)
      label != "key2"
