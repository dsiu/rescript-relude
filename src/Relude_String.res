...
")
let toNonWhitespace: string => option<string> = s => isWhitespace(s) ? None : Some(s)
...
")
let concatArray: array<string> => string = array =>
  Relude_Array_Instances.foldLeft((acc, str) => acc ++ str, "", array)

@ocaml.doc("
[String.concatList] is the [list] version of {!val:concatArray}.
")
let concatList: list<string> => string = list =>
  Relude_List_Instances.foldLeft((acc, str) => acc ++ str, "", list)
...
")
let makeWithIndex: (int, int => string) => string = (i, f) => {
...
")
let repeat: (int, string) => string = (i, str) => {
...
")
let charAtOrEmpty: (int, string) => string = (i, str) =>
  switch charAt(i, str) {
  | None => ""
  | Some(x) => x
  }
...
")
let toList: string => list<string> = str =>
...
")
let toArray: string => array<string> = str =>
...
")
let foldLeft: (('b, string) => 'b, 'b, string) => 'b = (f, init, str) =>
  Relude_List_Instances.foldLeft(f, init, toList(str))
...
")
let foldRight: ((string, 'b) => 'b, 'b, string) => 'b = (f, init, str) =>
  Relude_List_Instances.foldRight(f, init, toList(str))
...
")
let splitAt: (int, string) => (string, string) = (index, input) => (
  slice(0, index, input),
  sliceToEnd(index, input),
)
