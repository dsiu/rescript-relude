...
")
let make: 'a 'b 'c 'd 'e. ('a, 'b, 'c, 'd, 'e) => ('a, 'b, 'c, 'd, 'e) = (a, b, c, d, e) => (
  a,
  b,
  c,
  d,
...
")
let fromArray: 'a. array<'a> => option<('a, 'a, 'a, 'a, 'a)> = x =>
  switch x {
...
")
let apply: 'a 'b 'c 'd 'e 'f. (('a, 'b, 'c, 'd, 'e) => 'f, ('a, 'b, 'c, 'd, 'e)) => 'f = (
  f,
  (a, b, c, d, e),
) => f(a, b, c, d, e)
...
")
let first: 'a 'b 'c 'd 'e. (('a, 'b, 'c, 'd, 'e)) => 'a = ((a, _b, _c, _d, _e)) => a

@ocaml.doc("
Gets the second value of a tuple-5
")
let second: 'a 'b 'c 'd 'e. (('a, 'b, 'c, 'd, 'e)) => 'b = ((_a, b, _c, _d, _e)) => b

@ocaml.doc("
Gets the third value of a tuple-5
")
let third: 'a 'b 'c 'd 'e. (('a, 'b, 'c, 'd, 'e)) => 'c = ((_a, _b, c, _d, _e)) => c

@ocaml.doc("
Gets the fourth value of a tuple-5
")
let fourth: 'a 'b 'c 'd 'e. (('a, 'b, 'c, 'd, 'e)) => 'd = ((_a, _b, _c, d, _e)) => d

@ocaml.doc("
Gets the fifth value of a tuple-5
")
let fifth: 'a 'b 'c 'd 'e. (('a, 'b, 'c, 'd, 'e)) => 'e = ((_a, _b, _c, _d, e)) => e

let showBy: 'a 'b 'c 'd 'e. (
  'a => string,
  'b => string,
  'c => string,
  'd => string,
  'e => string,
  ('a, 'b, 'c, 'd, 'e),
) => string = (showA, showB, showC, showD, showE, (a, b, c, d, e)) =>
  "(" ++
  (showA(a) ++
  (", " ++
  (showB(b) ++
  (", " ++ (showC(c) ++ (", " ++ (showD(d) ++ (", " ++ (showE(e) ++ ")")))))))))
...
let eqBy: 'a 'b 'c 'd 'e. (
  ...
  ('a, 'b, 'c, 'd, 'e),
  ('a, 'b, 'c, 'd, 'e),
) => bool = (eqA, eqB, eqC, eqD, eqE, (a1, b1, c1, d1, e1), (a2, b2, c2, d2, e2)) =>
  eqA(a1, a2) && (eqB(b1, b2) && (eqC(c1, c2) && (eqD(d1, d2) && eqE(e1, e2))))
...
module type EQ_BY_F = (
  ...
  EqD: EQ,
  EqE: EQ,
  A: Relude_Interface.FUNCTION_1 with type b = (EqA.t, EqB.t, EqC.t, EqD.t, EqE.t),
) => (EQ with type t = A.a)
...
module EqBy: EQ_BY_F = (
  ...
  type t = A.a
  let eq = (t1, t2) => eqBy(EqA.eq, EqB.eq, EqC.eq, EqD.eq, EqE.eq, A.f(t1), A.f(t2))
}

let compareBy: 'a 'b 'c 'd 'e. (
...
module type ORD_BY_F = (
  ...
  OrdD: ORD,
  OrdE: ORD,
  A: Relude_Interface.FUNCTION_1 with type b = (OrdA.t, OrdB.t, OrdC.t, OrdD.t, OrdE.t),
) => (ORD with type t = A.a)
