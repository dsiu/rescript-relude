...
let moveLeftWithClamp: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveLeft |> Relude_Option.getOrElse(zipper)

@ocaml.doc("
Moves the focus as far as possible to the left
")
let rec moveLeftToStart: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveLeft |> Relude_Option.foldLazy(() => zipper, moveLeftToStart)
...
let moveLeftTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, zipper) =>
  zipper |> moveLeftTimes(times) |> Relude_Option.getOrElseLazy(() => zipper |> moveLeftToStart)
...
let moveRightWithClamp: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveRight |> Relude_Option.getOrElse(zipper)

@ocaml.doc("
Moves the focus as far as possible to the right
")
let rec moveRightToEnd: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveRight |> Relude_Option.foldLazy(() => zipper, moveRightToEnd)
...
let moveRightTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, zipper) =>
  zipper |> moveRightTimes(times) |> Relude_Option.getOrElseLazy(() => zipper |> moveRightToEnd)
...
let moveUpWithClamp: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveUp |> Relude_Option.getOrElse(zipper)
...
let moveUpTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, zipper) =>
  zipper |> moveUpTimes(times) |> Relude_Option.getOrElseLazy(() => zipper |> moveUpToTop)
...
let moveDownWithClamp: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveDown |> Relude_Option.getOrElse(zipper)
...
let rec moveDownToBottom: 'a. t<'a> => t<'a> = zipper =>
  zipper |> moveDown |> Relude_Option.foldLazy(() => zipper, moveDownToBottom)
...
let moveDownTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, zipper) =>
  zipper |> moveDownTimes(times) |> Relude_Option.getOrElseLazy(() => zipper |> moveDownToBottom)
...
let moveBy: 'a. (list<movement>, t<'a>) => option<t<'a>> = (moves, zipper) =>
  moves |> Relude_List.foldLeft(
    (zipperOpt, move) => zipperOpt |> Relude_Option.flatMap(moveOnceBy(move)),
    Some(zipper),
  )
  ...
  moves |> Relude_List.foldLeft(
    (zipperAccOpt, move) =>
      zipperAccOpt |> Relude_Option.flatMap(((zipper, acc)) =>
        zipper
        |> moveOnceBy(move)
        |> Relude_Option.map(nextZipper => (nextZipper, f(acc, nextZipper |> getFocusValue)))
      ),
    Some((zipper, init)),
  )
...
let map: 'a 'b. ('a => 'b, t<'a>) => t<'b> = (
  ...
  ancestors: ancestors |> Relude_List.map(((left, focus, right)) => (
...
let findInFocusAndChildren: 'a. ('a => bool, t<'a>) => option<t<'a>> = (pred, zipper) => {
  let rec dfs = zipper =>
    ...
    |> Relude_Option.orElseLazy(~fallback=() => moveDown(zipper) |> Relude_Option.flatMap(dfs))
    |> Relude_Option.orElseLazy(~fallback=() => moveRight(zipper) |> Relude_Option.flatMap(dfs))
...
and findLeftOrRight: 'a. (~checkFocus: bool=?, 'a => bool, t<'a>) => option<t<'a>> = (
  ...
  |> Relude_Option.orElseLazy(~fallback=() => zipper |> findRight(~checkFocus=false, pred))
...
and findUp: 'a. ('a => bool, t<'a>) => option<t<'a>> = (pred, zipper) =>
  ...
  |> Relude_Option.flatMap(parentZipper =>
    ...
    |> Relude_Option.orElseLazy(~fallback=() => parentZipper |> findUp(pred))
  )
...
and findDown: 'a. ('a => bool, t<'a>) => option<t<'a>> = (pred, zipper) =>
  ...
  |> Relude_Option.flatMap(childZipper =>
    childZipper
    |> findRight(pred)
    |> Relude_Option.orElseLazy(~fallback=() => childZipper |> findDown(pred))
  )
...
let insertTreeWithPushLeft: 'a. (Relude_Tree.t<'a>, t<'a>) => option<t<'a>> = (
  ...
  if ancestors |> Relude_List.isNotEmpty {
    Some({
      ancestors: ancestors,
      leftSiblings: list{{value: focus, children: children}, ...leftSiblings},
...
let insertTreeWithPushRight: 'a. (Relude_Tree.t<'a>, t<'a>) => option<t<'a>> = (
  ...
  if ancestors |> Relude_List.isNotEmpty {
    Some({
      ...
      leftSiblings: leftSiblings,
      focus: newTree |> Relude_Tree.getValue,
      rightSiblings: list{{value: focus, children: children}, ...rightSiblings},
      children: newTree |> Relude_Tree.getChildren,
    })
  } else {
    None
...
let showBy: 'a. ('a => string, t<'a>) => string = (
  ...
  let leftSiblingsStr = leftSiblings |> Relude_List.showBy(Relude_Tree.showBy(showA))

  let focusStr = showA(focus)

  let rightSiblingsStr = rightSiblings |> Relude_List.showBy(Relude_Tree.showBy(showA))

  let childrenStr = children |> Relude_List.showBy(Relude_Tree.showBy(showA))
