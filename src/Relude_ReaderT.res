...
module WithMonad = (M: MONAD) => {
  type t<'r, 'a> = ReaderT('r => M.t<'a>)

  let make: 'r 'a. ('r => M.t<'a>) => t<'r, 'a> = rToMA => ReaderT(rToMA)

  let runReaderT: 'r 'a. ('r, t<'r, 'a>) => M.t<'a> = (r, ReaderT(rToMA)) => rToMA(r)

  let mapReaderT: 'r 'a 'b. (M.t<'a> => M.t<'b>, t<'r, 'a>) => t<'r, 'b> = (
    maToMB,
    ReaderT(rToMA),
  ) => ReaderT(\"<<"(maToMB, rToMA))

  let withReaderT: 'r1 'r2 'a. ('r2 => 'r1, t<'r1, 'a>) => t<'r2, 'a> = (
    r2ToR1,
    ReaderT(r1ToMA),
  ) => ReaderT(\"<<"(r1ToMA, r2ToR1))

  let ask: 'r. t<'r, 'r> = ReaderT(r => M.pure(r))

  let asks: 'r 'a. ('r => 'a) => t<'r, 'a> = rToA => ReaderT(r => M.pure(rToA(r)))

  let local: 'r 'a. ('r => 'r, t<'r, 'a>) => t<'r, 'a> = (rToR, ReaderT(rToMA)) => ReaderT(
    r => rToMA(rToR(r)),
  )
