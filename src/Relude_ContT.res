...
module WithMonad = (M: MONAD) => {
  ...
  ")
  type t<'r, 'a> = ContT(('a => M.t<'r>) => M.t<'r>)

  @ocaml.doc("
  Constructs a ContT
  ")
  let make: (('a => M.t<'r>) => M.t<'r>) => t<'r, 'a> = onDone => ContT(onDone)

  @ocaml.doc("
  Runs the computation by providing a final continuation callback
  ")
  let runContT: 'a 'r. ('a => M.t<'r>, t<'r, 'a>) => M.t<'r> = (k, ContT(f)) => f(k)

  @ocaml.doc("
  Modify the underlying action in a ContT monad
  ")
  let mapContT: 'a 'r. (M.t<'r> => M.t<'r>, t<'r, 'a>) => t<'r, 'a> = (f, ContT(m)) => ContT(
    k => f(m(k)),
  )
  ...
  module WithResult = (R: TYPE) => {
    type t<'a> = t<R.t, 'a> // = | ContT(('a => M.t(R.t)) => M.t(R.t));
