...
")
let fromArray: 'a. array<'a> => option<('a, 'a)> = x =>
  switch x {
  | [a, b] => Some((a, b))
  | _ => None
  }
...
")
let showBy: 'a 'b. ('a => string, 'b => string, ('a, 'b)) => string = (showA, showB, (a, b)) =>
  "(" ++ (showA(a) ++ (", " ++ (showB(b) ++ ")")))
...
['a] and ['b]
")
let eqBy: 'a 'b. (('a, 'a) => bool, ('b, 'b) => bool, ('a, 'b), ('a, 'b)) => bool = (
  eqA,
  eqB,
  (a1, b1),
  (a2, b2),
...
module EqBy: EQ_BY_F = (
  ...
  type t = A.a
  let eq = (t1, t2) => eqBy(EqA.eq, EqB.eq, A.f(t1), A.f(t2))
}
...
let compareBy: 'a 'b. (
  ...
  ('a, 'b),
  ('a, 'b),
) => Relude_Ordering.t = (compareA, compareB, (a1, b1), (a2, b2)) =>
...
module OrdBy: ORD_BY_F = (
  ...
  let compare = (t1, t2) => compareBy(OrdA.compare, OrdB.compare, A.f(t1), A.f(t2))
}
