...
")
let errorNel: 'a 'e. 'e => t<'a, Relude_NonEmpty.List.t<'e>> = e => VError(
  Relude_NonEmpty.List.pure(e),
)

@ocaml.doc("
Puts an error into a NonEmptyArray on the error side of the validation
")
let errorNea: 'a 'e. 'e => t<'a, Relude_NonEmpty.Array.t<'e>> = e => VError(
  Relude_NonEmpty.Array.pure(e),
)
...
")
let tap: 'a 'e. ('a => unit, t<'a, 'e>) => t<'a, 'e> = (f, x) =>
  switch x {
  | VError(_) as e => e
  | VOk(a) as ok =>
    f(a)
...
")
let mapError: 'a 'e1 'e2. ('e1 => 'e2, t<'a, 'e1>) => t<'a, 'e2> = (f, x) =>
  switch x {
  | VOk(_) as ok => ok
  | VError(e) => VError(f(e))
  }
...
")
let mapErrorsNea: 'a 'e1 'e2. (
  'e1 => 'e2,
  t<'a, Relude_NonEmpty.Array.t<'e1>>,
...
")
let mapErrorsNel: 'a 'e1 'e2. (
  'e1 => 'e2,
  t<'a, Relude_NonEmpty.List.t<'e1>>,
...
")
let tapError: 'a 'e. ('e => unit, t<'a, 'e>) => t<'a, 'e> = (f, x) =>
  switch x {
  | VOk(_) as ok => ok
  | VError(e) as err =>
    f(e)
...
")
let bimap: 'a 'b 'e1 'e2. ('a => 'b, 'e1 => 'e2, t<'a, 'e1>) => t<'b, 'e2> = (f, g, x) =>
  switch x {
  | VOk(a) => VOk(f(a))
  | VError(e) => VError(g(e))
  }
...
")
let bitap: 'a 'e. ('a => unit, 'e => unit, t<'a, 'e>) => t<'a, 'e> = (f, g, x) =>
  switch x {
  | VOk(a) as ok =>
    f(a)
    ok
...
let alignWithAppendErrors: 'a 'b 'e. (
  ('e, 'e) => 'e,
  t<'a, 'e>,
  t<'b, 'e>,
) => t<Relude_Ior_Type.t<'a, 'b>, 'e> = (appendErrors, fa, fb) =>
  switch (fa, fb) {
...
let alignWithWithAppendErrors: 'a 'b 'c 'e. (
  ('e, 'e) => 'e,
...
")
let fromResult: result<'a, 'b> => t<'a, 'b> = x =>
  switch x {
  | Ok(a) => VOk(a)
  | Error(e) => VError(e)
  }
...
")
let toResult: t<'a, 'b> => result<'a, 'b> = x =>
  switch x {
  | VOk(a) => Ok(a)
  | VError(a) => Error(a)
  }
...
")
let fromOption: 'a 'e. ('e, option<'a>) => t<'a, 'e> = (defaultError, x) =>
  switch x {
  | Some(a) => ok(a)
...
")
let fromOptionLazy: 'a 'e. (unit => 'e, option<'a>) => t<'a, 'e> = (getDefaultError, x) =>
  switch x {
  | Some(a) => ok(a)
...
")
let fold: 'a 'e 'c. ('e => 'c, 'a => 'c, t<'a, 'e>) => 'c = (ec, ac, x) =>
  switch x {
  | VOk(a) => ac(a)
  | VError(e) => ec(e)
  }
...
module WithErrors = (Errors: SEMIGROUP_ANY, Error: TYPE) => {
  ...
  module Semialign: Relude_Interface.SEMIALIGN with type t<'a> = t<'a> = {
    ...
    let alignWith = (f, fa, fb) => alignWithWithAppendErrors(Errors.append, f, fa, fb)
  }
