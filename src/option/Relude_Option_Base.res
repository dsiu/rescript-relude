...
")
let filter: 'a. ('a => bool, option<'a>) => option<'a> = (fn, opt) =>
  Relude_Option_Instances.foldLeft((default, v) => fn(v) ? Some(v) : default, empty)(opt)

@ocaml.doc("
[Option.keep] is an alias for {!val:filter}.
")
let keep: 'a. ('a => bool, option<'a>) => option<'a> = (fn, opt) => filter(fn, opt)
...
")
let filterNot: 'a. ('a => bool, option<'a>) => option<'a> = (fn, opt) => filter(a => !fn(a), opt)
