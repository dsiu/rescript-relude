...
let align: 'a 'b. (option<'a>, option<'b>) => option<Relude_Ior_Type.t<'a, 'b>> = (fa, fb) =>
  switch (fa, fb) {
  | (None, None) => None
...
let alignWith: 'a 'b 'c. (Relude_Ior_Type.t<'a, 'b> => 'c, option<'a>, option<'b>) => option<'c> = (
  f,
  fa,
  fb,
) => map(f, align(fa, fb))
...
")
let alt: 'a. (option<'a>, option<'a>) => option<'a> = (fa1, fa2) =>
  switch fa1 {
  | Some(_) => fa1
  | None => fa2
  }
...
the right.
")
let altLazy: 'a. (option<'a>, unit => option<'a>) => option<'a> = (fa1, getFA2) =>
  switch fa1 {
  | Some(_) => fa1
  | None => getFA2()
  }
...
")
let eqBy: (('a, 'a) => bool, option<'a>, option<'a>) => bool = (innerEq, a, b) =>
  switch (a, b) {
...
")
let eq = (type a, showA: module(EQ with type t = a), fa1: option<a>, fa2: option<a>): bool => {
  module Eq = BsBastet.Option.Eq(unpack(showA))
  Eq.eq(fa1, fa2)
}

module Eq: BsBastet.Option.EQ_F = (EqA: EQ) => {
  type t = option<EqA.t>
...
")
let showBy: 'a. ('a => string, option<'a>) => string = (showA, x) =>
  switch x {
  | Some(a) => "Some(" ++ (showA(a) ++ ")")
  | None => "None"
  }
...
")
let show = (type a, showA: module(SHOW with type t = a), fa: option<a>): string => {
  module Show = BsBastet.Option.Show(unpack(showA))
  Show.show(fa)
}
