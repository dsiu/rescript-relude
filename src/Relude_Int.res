...
")
let modulo: (int, int) => int = (a, b) => mod(a, b)

@ocaml.doc("
Finds the quotient and remainder of an integer division
")
let divideWithModulo: (int, int) => (int, int) = (a, b) => (divide(a, b), modulo(a, b))
...
let rec rangeAsList = (start: int, end_: int): list<int> =>
  if start >= end_ {
    ...
    list{start, ...rangeAsList(start + 1, end_)}
  }
...
let rec rangeAsArray: (int, int) => array<int> = (start, end_) =>
  if start >= end_ {
    ...
    Relude_Array_Instances.concat([start], rangeAsArray(start + 1, end_))
  }
