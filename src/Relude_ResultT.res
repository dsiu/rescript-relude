...
module WithMonad = (M: MONAD) => {
  type t<'a, 'e> = ResultT(M.t<result<'a, 'e>>)

  let make: 'a 'e. M.t<result<'a, 'e>> => t<'a, 'e> = mResult => ResultT(mResult)

  let runResultT: 'a 'e. t<'a, 'e> => M.t<result<'a, 'e>> = (ResultT(mResult)) => mResult

  let withResultT: 'a 'e1 'e2. ('e1 => 'e2, t<'a, 'e1>) => t<'a, 'e2> = (
    e1ToE2,
    ResultT(mResultE1),
  ) => ResultT(M.map(resultE1 => Relude_Result.mapError(e1ToE2, resultE1), mResultE1))

  let mapResultT: 'a 'b 'e. (M.t<result<'a, 'e>> => M.t<result<'b, 'e>>, t<'a, 'e>) => t<'b, 'e> = (
    mResultAToMResultB,
    ResultT(mResultA),
  ) => ResultT(mResultAToMResultB(mResultA))

  let fromResult: 'a 'e. result<'a, 'e> => t<'a, 'e> = result => ResultT(M.pure(result))
    ...
    M.map(
      resultA =>
        Relude_Result.flatMap(
          a => aToBool(a) ? Relude_Result.pure(success) : Relude_Result.error(err),
          resultA,
        ),
      mResultA,
    ),
  ...
  let condError: 'a 'b 'e. ('a => bool, 'e, t<'a, 'e>) => t<'a, 'e> = (
    ...
    M.map(
      resultA =>
        Relude_Result.flatMap(
          a => aToBool(a) ? Relude_Result.pure(a) : Relude_Result.error(err),
          resultA,
        ),
      mResultA,
    ),
  ...
  let semiflatMap: 'a 'b 'e. ('a => M.t<'b>, t<'a, 'e>) => t<'b, 'e> = (aToMB, resultTA) =>
    bind(resultTA, a => liftF(aToMB(a)))
