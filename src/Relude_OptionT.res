...
module WithMonad = (M: MONAD) => {
  type t<'a> = OptionT(M.t<option<'a>>)

  let make: 'a. M.t<option<'a>> => t<'a> = mOptionA => OptionT(mOptionA)

  let runOptionT: 'a. t<'a> => M.t<option<'a>> = (OptionT(mOptionA)) => mOptionA
  ...
  let fromOption: 'a. option<'a> => t<'a> = optionA => OptionT(M.pure(optionA))
  ...
  let subflatMap: 'a 'b. ('a => option<'b>, t<'a>) => t<'b> = (aToB, OptionT(mOptionA)) => OptionT(
  ...
  let apply: 'a 'b. (t<'a => 'b>, t<'a>) => t<'b> = (
    ...
    M.apply(
      M.map(optionAToB => optionA => Relude_Option.apply(optionAToB, optionA), mOptionAToB),
      mOptionA,
    ),
  )
  ...
  let semiflatMap: 'a 'b. ('a => M.t<'b>, t<'a>) => t<'b> = (aToMB, optionTA) =>
    bind(optionTA, a => liftF(aToMB(a)))
