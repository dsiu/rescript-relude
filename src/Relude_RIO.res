...
module WithError = (ERR: TYPE) => {
  ...
  type t<'r, 'a> = RIO('r => M.t<'a>)

  let make: 'r 'a. ('r => M.t<'a>) => t<'r, 'a> = rToMA => RIO(rToMA)

  let runRIO: 'r 'a. ('r, t<'r, 'a>) => M.t<'a> = (r, RIO(rToMA)) => rToMA(r)

  let mapRIO: 'r 'a 'b. (M.t<'a> => M.t<'b>, t<'r, 'a>) => t<'r, 'b> = (maToMB, RIO(rToMA)) => RIO(
    \"<<"(maToMB, rToMA),
  )

  let withRIO: 'r1 'r2 'a. ('r2 => 'r1, t<'r1, 'a>) => t<'r2, 'a> = (r2ToR1, RIO(r1ToMA)) => RIO(
    \"<<"(r1ToMA, r2ToR1),
  )

  let ask: 'r. t<'r, 'r> = RIO(r => M.pure(r))

  let asks: 'r 'a. ('r => 'a) => t<'r, 'a> = rToA => RIO(r => M.pure(rToA(r)))

  let local: 'r 'a. ('r => 'r, t<'r, 'a>) => t<'r, 'a> = (rToR, RIO(rToMA)) => RIO(
    r => rToMA(rToR(r)),
  )
  ...
  let semiflatMap: 'r 'a 'b. ('a => M.t<'b>, t<'r, 'a>) => t<'r, 'b> = (aToMA, RIO(rToMA)) => RIO(
    r => M.flat_map(rToMA(r), a => aToMA(a)),
  )

  let catchError: 'r 'a 'b. (M.e => M.t<'a>, t<'r, 'a>) => t<'r, 'a> = (eToMa, RIO(rToMA)) => RIO(
    r => M.catchError(eToMa, rToMA(r)),
  )

  let throwError: 'r 'a 'b. M.e => t<'r, 'a> = e => RIO(_ => M.throwError(e))

  let mapError: 'r 'a 'b. (M.e => M.e, t<'r, 'a>) => t<'r, 'a> = (eToE, RIO(rToMA)) => RIO(
    r => IOE.mapError(eToE, rToMA(r)),
  )
