...
module OrdExtensions = (O: ORD) => {
  ...
  let compareAsInt: (O.t, O.t) => int = (a, b) => Relude_Ord.compareAsIntBy(O.compare, a, b)

  let min: (O.t, O.t) => O.t = (a, b) => Relude_Ord.minBy(O.compare, a, b)

  let max: (O.t, O.t) => O.t = (a, b) => Relude_Ord.maxBy(O.compare, a, b)

  let lessThan: (O.t, O.t) => bool = (a, b) => Relude_Ord.lessThanBy(O.compare, a, b)

  let lt: (O.t, O.t) => bool = lessThan

  let lessThanOrEq: (O.t, O.t) => bool = (a, b) => Relude_Ord.lessThanOrEqBy(O.compare, a, b)

  let lte: (O.t, O.t) => bool = lessThanOrEq

  let greaterThan: (O.t, O.t) => bool = (a, b) => Relude_Ord.greaterThanBy(O.compare, a, b)

  let gt: (O.t, O.t) => bool = greaterThan

  let greaterThanOrEq: (O.t, O.t) => bool = (a, b) => Relude_Ord.greaterThanOrEqBy(O.compare, a, b)

  let gte: (O.t, O.t) => bool = greaterThanOrEq

  let clamp: (~min: O.t, ~max: O.t, O.t) => O.t = (~min, ~max, v) =>
    Relude_Ord.clampBy(O.compare, ~min, ~max, v)

  let between: (~min: O.t, ~max: O.t, O.t) => bool = (~min, ~max, v) =>
    Relude_Ord.betweenBy(O.compare, ~min, ~max, v)

  module OrdRingExtensions = (R: RING with type t = O.t) => {
    let abs: R.t => R.t = v => Relude_Ord.abs(module(O), module(R), v)

    let signum: R.t => R.t = v => Relude_Ord.signum(module(O), module(R), v)
  }
  ...
  ")
  module OrdNamed = {
    let lessThan = (~compareTo, a) => Relude_Ord.lessThanBy(O.compare, a, compareTo)

    let lessThanOrEq = (~compareTo, a) => Relude_Ord.lessThanOrEqBy(O.compare, a, compareTo)

    let greaterThan = (~compareTo, a) => Relude_Ord.greaterThanBy(O.compare, a, compareTo)

    let greaterThanOrEq = (~compareTo, a) => Relude_Ord.greaterThanOrEqBy(O.compare, a, compareTo)
  ...
  module type ORD_BY_F = (A: Relude_Interface.FUNCTION_1 with type b = O.t) =>
  (ORD with type t = A.a)
  ...
  ")
  module OrdBy: ORD_BY_F = (A: Relude_Interface.FUNCTION_1 with type b = O.t) => {
    ...
    let compare: (t, t) => ordering = (b1, b2) => O.compare(A.f(b1), A.f(b2))
  }
}
