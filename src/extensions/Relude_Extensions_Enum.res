...
module EnumExtensions = (E: Relude_Interface.ENUM) => {
  ...
  values (inclusive)
  ")
  let fromToAsList: (~start: E.t, ~finish: E.t) => list<E.t> = (~start, ~finish) => {
  ...
  ")
  let upFromAsList: E.t => list<E.t> = start => {
    let rec go: (list<E.t>, E.t) => list<E.t> = (acc, current) =>
      switch E.succ(current) {
      | Some(next) => go(listAppend(current, acc), next)
      | None => listAppend(current, acc)
      }
    switch E.succ(start) {
    | Some(next) => go(list{}, next)
    | None => list{}
    }
  }
  ...
  (including the start). Warning: the resulting list could be unbounded here.
  ")
  let upFromIncludingAsList: E.t => list<E.t> = start => {
    let rec go: (list<E.t>, E.t) => list<E.t> = (acc, current) =>
      switch E.succ(current) {
      | Some(next) => go(listAppend(current, acc), next)
      | None => listAppend(current, acc)
  ...
  including the start). Warning: the resulting list could be unbounded here.
  ")
  let downFromAsList: E.t => list<E.t> = start => {
    let rec go: (list<E.t>, E.t) => list<E.t> = (acc, current) =>
      switch E.pred(current) {
      | Some(prev) => go(listAppend(current, acc), prev)
      | None => listAppend(current, acc)
  ...
  (including the start). Warning: the resulting list could be unbounded here.
  ")
  let downFromIncludingAsList: E.t => list<E.t> = start => {
    let rec go: (list<E.t>, E.t) => list<E.t> = (acc, current) =>
      switch E.pred(current) {
      | Some(prev) => go(listAppend(current, acc), prev)
      | None => listAppend(current, acc)
