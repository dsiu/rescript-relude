...
module Result = {
  ...
  ")
  let traverse = (type e, f: 'a => result<'b, e>, list: list<'a>): result<list<'b>, e> => {
    module ResultE = Relude_Result.WithError({
      type t = e
    })
    module TraverseResult = Relude_List_Instances.Traversable(ResultE.Applicative)
...
module IO = {
  ...
  let sequence = (type e, xs: list<Relude_IO.t<'a, e>>): Relude_IO.t<list<'a>, e> => {
    module IoE = Relude_IO.WithError({
      type t = e
    })
    module TraverseIO = Relude_List_Instances.Traversable(IoE.Applicative)
...
module Validation = {
  ...
  module TraversableWithErrorsAsList = (Error: TYPE) => Traversable(
    Relude_List_Instances.SemigroupAny,
    Error,
  )

  module TraversableWithErrorsAsListOfStrings = TraversableWithErrorsAsList({
    type t = string
  })

  module TraversableWithErrorsAsNonEmptyList = (Error: TYPE) => Traversable(
    Relude_NonEmpty.List.SemigroupAny,
    Error,
  )
  ...
  let traverse = (
    ...
    f: a => result<
      b,
      e,
    > /* Each a produces a Result with a success value or a single error value */,
    list: list<a>,
