...
module WithSequence = (S: Relude_Interface.SEQUENCE) => {
  ...
  module Traversable: TRAVERSABLE_F = (A: APPLICATIVE) => {
    ...
    let traverse: 'a 'b. ('a => A.t<'b>, t<'a>) => A.t<t<'b>> = (f, Zipper(left, focus, right)) =>
      \"<*>"(
        \"<*>"(\"<$>"(make, STraversable.traverse(f, left)), f(focus)),
  ...
  let toNonEmptyArray: 'a. t<'a> => Relude_NonEmpty.Array.t<'a> = (Zipper(left, focus, right)) =>
    ...
    |> Relude_Option.foldLazy(
      ...
      ((lh, lt)) =>
        Relude_NonEmpty.Array.make(lh, S.concat(lt |> S.append(focus), right) |> S.toArray),
    )
  ...
  let toNonEmptyList: 'a. t<'a> => Relude_NonEmpty.List.t<'a> = (Zipper(left, focus, right)) =>
    ...
    |> Relude_Option.foldLazy(
      ...
      ((lh, lt)) =>
        Relude_NonEmpty.List.make(lh, S.concat(lt |> S.append(focus), right) |> S.toList),
    )
  ...
  ")
  let isAtIndex: 'a. (int, t<'a>) => bool = (target, z) =>
    z |> zipWithIndex |> getFocus |> snd == target
  ...
  let moveLeft: 'a. t<'a> => option<t<'a>> = (Zipper(left, focus, right)) =>
    left
    |> S.uncons
    |> Relude_Option.map(((leftH, leftT)) => Zipper(leftT, leftH, S.prepend(focus, right)))
  ...
  let moveRight: 'a. t<'a> => option<t<'a>> = (Zipper(left, focus, right)) =>
    right
    |> S.uncons
    |> Relude_Option.map(((rightH, rightT)) => Zipper(S.prepend(focus, left), rightH, rightT))

  @ocaml.doc("
  Moves the focus one item to the left, unless we are at the start.
  ")
  let moveLeftWithClamp: 'a. t<'a> => t<'a> = z => moveLeft(z) |> Relude_Option.getOrElse(z)

  @ocaml.doc("
  Moves the focus one item to the right, unless we are at the end.
  ")
  let moveRightWithClamp: 'a. t<'a> => t<'a> = z => moveRight(z) |> Relude_Option.getOrElse(z)
  ...
  let moveStart: 'a. t<'a> => t<'a> = (Zipper(left, focus, right) as z) =>
    ...
    |> Relude_Option.foldLazy(
      () => z,
      ((leftH, leftT)) => Zipper(S.emptyLazy(), leftH, S.concat(leftT |> S.append(focus), right)),
    )
  ...
  let moveLeftWithWrap: 'a. t<'a> => t<'a> = z =>
    moveLeft(z) |> Relude_Option.getOrElseLazy(() => moveEnd(z))
  ...
  let moveRightWithWrap: 'a. t<'a> => t<'a> = z =>
    moveRight(z) |> Relude_Option.getOrElseLazy(() => moveStart(z))
  ...
  let moveLeftTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, z) =>
    moveLeftTimes(times, z) |> Relude_Option.getOrElseLazy(() => moveStart(z))
  ...
  let moveRightTimesWithClamp: 'a. (int, t<'a>) => t<'a> = (times, z) =>
    moveRightTimes(times, z) |> Relude_Option.getOrElseLazy(() => moveEnd(z))
  ...
  let moveToIndexWithMod: 'a. (int, t<'a>) => t<'a> = (target, z) => {
    let modTarget = mod(target, z |> length)
  ...
  let findBy = (~checkFocus=true, f: 'a => bool, z: t<'a>): option<t<'a>> =>
    findLeftBy(~checkFocus, f, z) |> Relude_Option.orElseLazy(~fallback=() =>
  ...
  let findItem = (
    ...
    findItemLeft(~checkFocus, eq, item, zipper) |> Relude_Option.orElseLazy(~fallback=() =>
  ...
  let deleteWithPullLeft: 'a. t<'a> => option<t<'a>> = (Zipper(left, _, right)) =>
    left |> S.uncons |> Relude_Option.map(((leftH, leftT)) => Zipper(leftT, leftH, right))
  ...
  let deleteWithPullRight: 'a. t<'a> => option<t<'a>> = (Zipper(left, _, right)) =>
    right |> S.uncons |> Relude_Option.map(((rightH, rightT)) => Zipper(left, rightH, rightT))
  ...
  let deleteWithPullLeftOrRight: 'a. t<'a> => option<t<'a>> = z =>
    deleteWithPullLeft(z) |> Relude_Option.orElseLazy(~fallback=() => deleteWithPullRight(z))
