...
")
let reloadingOk: 'a 'e. 'a => t<'a, 'e> = a => Relude_AsyncData.reloading(Ok(a))

@ocaml.doc("
Constructs a Reloading(Error(_)) value
")
let reloadingError: 'a 'e. 'e => t<'a, 'e> = e => Relude_AsyncData.reloading(Error(e))

@ocaml.doc("
Constructs a Complete(Ok(_)) value
")
let completeOk: 'a 'e. 'a => t<'a, 'e> = a => Relude_AsyncData.complete(Ok(a))

@ocaml.doc("
Constructs a Complete(Error(_)) value
")
let completeError: 'a 'e. 'e => t<'a, 'e> = e => Relude_AsyncData.complete(Error(e))
...
")
let isReloadingOk: 'a 'e. t<'a, 'e> => bool = x =>
  switch x {
  | Init => false
  | Loading => false
  | Reloading(Error(_)) => false
...
")
let isReloadingError: 'a 'e. t<'a, 'e> => bool = x =>
  switch x {
  | Init => false
  | Loading => false
  | Reloading(Error(_)) => true
...
")
let isCompleteOk: 'a 'e. t<'a, 'e> => bool = x =>
  switch x {
  | Init => false
  | Loading => false
  | Reloading(Error(_)) => false
...
")
let isCompleteError: 'a 'e. t<'a, 'e> => bool = x =>
  switch x {
  | Init => false
  | Loading => false
  | Reloading(Error(_)) => false
...
")
let getReloadingOk: 'a 'e. t<'a, 'e> => option<'a> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(Error(_)) => None
...
")
let getReloadingError: 'a 'e. t<'a, 'e> => option<'e> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(Error(x)) => Some(x)
...
")
let getCompleteOk: 'a 'e. t<'a, 'e> => option<'a> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(Error(_)) => None
...
")
let getCompleteError: 'a 'e. t<'a, 'e> => option<'e> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(Error(_)) => None
...
")
let mapError: 'a 'e1 'e2. ('e1 => 'e2, t<'a, 'e1>) => t<'a, 'e2> = (f, fa) =>
  switch fa {
  | Init => Init
  | Loading => Loading
...
")
let tapReloading: 'a 'e. (
  result<'a, 'e> => unit,
  t<'a, 'e>,
) => t<'a, 'e> = Relude_AsyncData.tapReloading
...
let tapComplete: 'a 'e. (
  result<'a, 'e> => unit,
  t<'a, 'e>,
) => t<'a, 'e> = Relude_AsyncData.tapComplete
...
let tapNotEmpty: 'a 'e. (
  result<'a, 'e> => unit,
  t<'a, 'e>,
) => t<'a, 'e> = Relude_AsyncData.tapNotEmpty

@ocaml.doc("
Applies a side effect function if the value is empty or not-empty
")
let tapByValue: 'a 'e. (unit => unit, result<'a, 'e> => unit, t<'a, 'e>) => t<'a, 'e> = (
  ifEmpty,
  ifNotEmpty,
  fa,
) =>
...
")
let tapOk: 'a 'e. ('a => unit, t<'a, 'e>) => t<'a, 'e> = (ifOk, fa) =>
  switch fa {
  | Init
  | Loading
  | Reloading(Error(_))
...
")
let tapError: 'a 'e. ('e => unit, t<'a, 'e>) => t<'a, 'e> = (ifError, fa) =>
  switch fa {
  | Init
  | Loading
  | Reloading(Ok(_))
...
")
let flatten: 'a 'e. t<t<'a, 'e>, 'e> => t<'a, 'e> = mma => flatMap(a => a, mma)
...
")
let fold: 'a 'e 'b. ('b, 'b, result<'a, 'e> => 'b, result<'a, 'e> => 'b, t<'a, 'e>) => 'b = (
  initValue,
  loadingValue,
  onReloading,
  onComplete,
...
let foldLazy: 'a 'e 'b. (
  ...
  result<'a, 'e> => 'b,
  t<'a, 'e>,
) => 'b = (onInit, onLoading, onReloading, onComplete, fa) =>
  switch fa {
  | Init => onInit()
  | Loading => onLoading()
  | Reloading(result) => onReloading(result)
...
")
let fromAsyncData: 'a 'e. Relude_AsyncData.t<'a> => t<'a, 'e> = x =>
  switch x {
  | Init => Init
  | Loading => Loading
...
")
let toAsyncData: 'a. t<'a, 'a> => Relude_AsyncData.t<'a> = x =>
  switch x {
  | Init => Init
  | Loading => Loading
