...
module WithMonad = (M: MONAD) => {
  ...
  let mapStateT: 's 'a 'b. (M.t<('a, 's)> => M.t<('b, 's)>, t<'a, 's>) => t<'b, 's> = (
    mASToMBS,
    StateT(sToMAS),
  ) => StateT(\"<<"(mASToMBS, sToMAS))

  @ocaml.doc("
  Modifies the state of a StateT
  ")
  let withStateT: 's 'a. ('s => 's, t<'a, 's>) => t<'a, 's> = (sToS, StateT(sToMAS)) => StateT(
    \"<<"(sToMAS, sToS),
  )
  ...
  ")
  let gets: 's 'a. ('s => 'a) => t<'a, 's> = sToA => StateT(s => M.pure((sToA(s), s)))
  ...
  ")
  let modify: 's. ('s => 's) => t<'s, 's> = sToS => StateT(
    s => {
      let s' = sToS(s)
      M.pure((s', s'))
    },
