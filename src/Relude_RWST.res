...
module WithMonad = (M: MONAD) => {
  ...
  type t<'a, 'r, 's, 'w> = RWST(('r, 's) => M.t<RWSResult.t<'a, 's, 'w>>)
  ...
  let applyWithAppendLog: 'a 'b 'r 's 'w. (
    ...
    t<'a => 'b, 'r, 's, 'w>,
    t<'a, 'r, 's, 'w>,
  ) => t<'b, 'r, 's, 'w> = (appendLog, RWST(runAToB), RWST(runA)) => RWST(
    (r, s) => {
      let mAToB = runAToB(r, s)
      let mA = runA(r, s)
      let f = (RWSResult(aToB, _s1, w1), RWSResult(a, s2, w2)) => RWSResult(
        aToB(a),
        s2,
        appendLog(w1, w2),
      )
  ...
  let pureWithEmptyLog: 'a 'r 's 'w. ('w, 'a) => t<'a, 'r, 's, 'w> = (w, a) => RWST(
    (_r, s) => M.pure(RWSResult(a, s, w)),
  )

  let bindWithAppendLog: 'a 'b 'r 's 'w. (
    ('w, 'w) => 'w,
    t<'a, 'r, 's, 'w>,
    'a => t<'b, 'r, 's, 'w>,
  ) => t<'b, 'r, 's, 'w> = (appendLog, RWST(runA), aToRWSTB) => RWST(
    (r1, s1) =>
  ...
  module WithEnvAndStateAndLog = (R: TYPE, S: TYPE, Log: Relude_WriterT.WriterLog.LOG) => {
    let runRWST = runRWST
