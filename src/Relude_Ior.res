...
")
let getThis: 'a 'b. t<'a, 'b> => option<'a> = x =>
  switch x {
  | This(a) => Some(a)
  | That(_) => None
  | Both(a, _) => Some(a)
...
")
let getThat: 'a 'b. t<'a, 'b> => option<'b> = x =>
  switch x {
  | This(_) => None
  | That(b) => Some(b)
  | Both(_, b) => Some(b)
...
")
let partition: 'a 'b. list<t<'a, 'b>> => (list<'a>, list<'b>, list<('a, 'b)>) = iors =>
  Relude_List.foldRight((ior, (as_, bs, boths)) =>
    switch ior {
    | This(a) => (list{a, ...as_}, bs, boths)
    | That(b) => (as_, list{b, ...bs}, boths)
    | Both(a, b) => (as_, bs, list{(a, b), ...boths})
    }
  , (list{}, list{}, list{}), iors)
...
")
let catThis: 'a 'b. list<t<'a, 'b>> => list<'a> = iors => Relude_List.foldRight((ior, acc) =>
    switch ior {
    | This(a) => list{a, ...acc}
    | That(_) => acc
    | Both(a, _) => list{a, ...acc}
...
")
let catThat: 'a 'b. list<t<'a, 'b>> => list<'b> = iors => Relude_List.foldRight((ior, acc) =>
    switch ior {
    | This(_) => acc
    | That(b) => list{b, ...acc}
    | Both(_, b) => list{b, ...acc}
...
")
let mapThat: 'a 'b 'c. ('b => 'c, t<'a, 'b>) => t<'a, 'c> = (f, fa) =>
  switch fa {
  | This(_) as a => a
  | That(b) => That(f(b))
  | Both(a, b) => Both(a, f(b))
...
")
let tapThis: 'a 'b. ('a => unit, t<'a, 'b>) => t<'a, 'b> = (ifThis, fa) =>
  switch fa {
  | This(a) =>
    ifThis(a)
    fa
...
")
let tapThat: 'a 'b. ('b => unit, t<'a, 'b>) => t<'a, 'b> = (ifThat, fa) =>
  switch fa {
  | This(_) => fa
  | That(e) =>
    ifThat(e)
...
")
let tapBoth: 'a 'b. (('a, 'b) => unit, t<'a, 'b>) => t<'a, 'b> = (ifBoth, fa) =>
  switch fa {
  | This(_) => fa
  | That(_) => fa
  | Both(a, e) =>
...
")
let tapThisOrBoth: 'a 'b. ('a => unit, t<'a, 'b>) => t<'a, 'b> = (ifThis, fa) =>
  switch fa {
  | This(a) =>
    ifThis(a)
    fa
...
")
let tapThatOrBoth: 'a 'b. ('b => unit, t<'a, 'b>) => t<'a, 'b> = (ifThat, fa) =>
  switch fa {
  | This(_) => fa
  | That(e) =>
    ifThat(e)
...
Applies a monadic function to the success channel of the Ior
")
let flatMap: ('a => t<'c, 'b>, t<'a, 'b>) => t<'c, 'b> = (f, fa) => bind(fa, f)
...
")
let toTuple: 'a 'b. ('a, 'b, t<'a, 'b>) => ('a, 'b) = (defaultA, defaultB, ior) =>
  switch ior {
  | This(a) => (a, defaultB)
  | That(b) => (defaultA, b)
  | Both(a, b) => (a, b)
...
")
let merge: 'a. (('a, 'a) => 'a, t<'a, 'a>) => 'a = (f, ior) =>
  switch ior {
  | This(a) => a
  | That(a) => a
  | Both(a1, a2) => f(a1, a2)
