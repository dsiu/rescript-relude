...
")
let getReloading: 'a. t<'a> => option<'a> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(a) => Some(a)
...
")
let getComplete: 'a. t<'a> => option<'a> = x =>
  switch x {
  | Init => None
  | Loading => None
  | Reloading(_) => None
...
")
let foldByValue: 'a 'b. ('b, 'a => 'b, t<'a>) => 'b = (defaultValue, onValue, fa) =>
...
there is no data, or function to handle when there is data.
")
let foldByValueLazy: 'a 'b. (unit => 'b, 'a => 'b, t<'a>) => 'b = (onNoValue, onValue, fa) =>
...
")
let tapInit: 'a. (unit => unit, t<'a>) => t<'a> = (ifInit, fa) =>
  switch fa {
  | Init =>
    ifInit()
    fa
...
")
let tapLoading: 'a. (unit => unit, t<'a>) => t<'a> = (ifLoading, fa) =>
  switch fa {
  | Init => fa
  | Loading =>
    ifLoading()
...
")
let tapReloading: 'a. ('a => unit, t<'a>) => t<'a> = (ifReloading, fa) =>
  switch fa {
  | Init => fa
  | Loading => fa
  | Reloading(a) =>
...
")
let tapComplete: 'a. ('a => unit, t<'a>) => t<'a> = (ifComplete, fa) =>
  switch fa {
  | Init => fa
  | Loading => fa
  | Reloading(_) => fa
...
")
let tapEmpty: 'a. (unit => unit, t<'a>) => t<'a> = (ifEmpty, fa) =>
  switch fa {
  | Init
  | Loading =>
    ifEmpty()
...
")
let tapNotEmpty: 'a. ('a => unit, t<'a>) => t<'a> = (ifNotEmpty, fa) =>
  switch fa {
  | Init
  | Loading => fa
  | Reloading(a)
...
")
let tapByValue: 'a. (unit => unit, 'a => unit, t<'a>) => t<'a> = (ifEmpty, ifNotEmpty, fa) =>
  switch fa {
  | Init
  | Loading =>
    ifEmpty()
...
")
let eqBy: 'a. (('a, 'a) => bool, t<'a>, t<'a>) => bool = (innerEq, a, b) =>
  switch (a, b) {
  | (Init, Init)
...
")
let showBy: 'a. ('a => string, t<'a>) => string = (showA, fa) =>
  switch fa {
  | Init => "Init"
  | Loading => "Loading"
