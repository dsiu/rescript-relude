...
module WriterLog = {
  ...
  module List = {
    ...
    module WithEntry: LIST_LOG = (Entry: TYPE) => {
      type t = list<Entry.t>
  ...
  ")
  module Array = {
    module type ARRAY_LOG = (Entry: TYPE) => (LOG with type t = array<Entry.t>)

    module WithEntry: ARRAY_LOG = (Entry: TYPE) => {
...
module WithMonad = (Monad: MONAD) => {
  ...
  let make: 'w 'a. Monad.t<('a, 'w)> => t<'a, 'w> = mWriter => WriterT(mWriter)

  @ocaml.doc("
  Run the writer, and get the result value a, and the log w
  ")
  let runWriterT: 'w 'a. t<'a, 'w> => Monad.t<('a, 'w)> = (WriterT(mWriter)) => mWriter
  ...
  ")
  let listen: 'w 'a. t<'a, 'w> => t<('a, 'w), 'w> = (WriterT(mAW)) => WriterT(
    Monad.map(((a, w)) => ((a, w), w), mAW),
  )

  @ocaml.doc("
  Applies a log-modifying function provided in the result tuple to the log
  ")
  let pass: 'w 'a. t<('a, 'w => 'w), 'w> => t<'a, 'w> = (WriterT(m)) => WriterT(
    Monad.map((((a, wToW), w)) => (a, wToW(w)), m),
  )
  ...
  Modifies the log
  ")
  let censor: 'w 'a. ('w => 'w, t<'a, 'w>) => t<'a, 'w> = (wToW, WriterT(mAW)) =>
  ...
  ")
  let pureWithEmptyLog: 'w 'a. ('w, 'a) => t<'a, 'w> = (empty, a) => WriterT(Monad.pure((a, empty)))
